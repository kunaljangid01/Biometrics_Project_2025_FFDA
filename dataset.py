...
transform_options['PoisonPubFig'] = transform_options['PubFig']
...
class DatasetGenerator():
    def __init__(self, train_batch_size=128, eval_batch_size=256, num_of_workers=4,
        ...
        if train_data_type == 'CIFAR10':
            ...
            # train_dataset = PoisonPubFig(root=train_data_path, transform=train_transform,
            #                             poison_rate=poison_rate, perturb_tensor_filepath=perturb_tensor_filepath,
            #                             patch_location=patch_location, seed=seed, perturb_type=perturb_type,
            #                             add_uniform_noise=add_uniform_noise)
        
        elif train_data_type == 'PoisonPubFig':
            # train_dataset = datasets.ImageFolder(root=train_data_path, transform=train_transform)
            # test_dataset = datasets.ImageFolder(root=test_data_path, transform=test_transform)
            # train_dataset = PoisonPubFig(root=train_data_path, transform=train_transform,
            #                             poison_rate=poison_rate, perturb_tensor_filepath=perturb_tensor_filepath,
        ...
        # print("test_dataset im in test_data_type: ",test_data_type)
        ...
        if test_data_type == 'CIFAR10':
            ...
            test_dataset = PoisonCIFAR10(root=test_data_path, train=False, transform=test_transform,
                                         ...
                                         poison_classwise=poison_classwise,
                                         poison_classwise_idx=poison_classwise_idx)
        elif test_data_type == 'PubFig':
            # train_dataset = datasets.ImageFolder(root=train_data_path, transform=train_transform)
            test_dataset = datasets.ImageFolder(root=test_data_path, transform=test_transform)
            
        elif test_data_type == 'PoisonPubFig':
            # print("test_dataset im in")
            # test_dataset = PoisonPubFig(root=test_data_path, transform=test_transform,
            #                             poison_rate=poison_rate, perturb_tensor_filepath=perturb_tensor_filepath,
            #                             patch_location=patch_location, seed=seed, perturb_type=perturb_type,
            #                             add_uniform_noise=add_uniform_noise)       
            ...
            pass
        elif test_data_type == 'FaceScrub' or test_data_type == 'WebFace': # or test_data_type == 'PubFig':
            pass
        else:
    ...
    def _split_validation_set(self, train_portion, train_shuffle=True, train_drop_last=True):
        ...
        elif self.train_data_type == 'WebFace' or self.train_data_type == 'PubFig' or isinstance(self.datasets['train_dataset'], datasets.ImageFolder):
            data, targets = list(zip(*self.datasets['train_dataset'].samples))
            datasplit = train_test_split(data, targets, test_size=1-train_portion,
                                        train_size=train_portion, shuffle=True, stratify=targets)
            train_D, valid_D, train_L, valid_L = datasplit
...
# class PoisonPubFig(datasets.ImageFolder):
#     def __init__(self, root, transform=None, poison_rate=1.0, perturb_tensor_filepath=None,
#                  seed=0, perturb_type='classwise', patch_location='center',
#                  add_uniform_noise=False):
#         super(PoisonPubFig, self).__init__(root=root, transform=transform)
#         np.random.seed(seed)
#         self.perturb_tensor = torch.load(perturb_tensor_filepath, map_location=device)
#         self.perturb_tensor = self.perturb_tensor.mul(255).clamp_(0, 255).permute(0, 2, 3, 1).to('cpu').numpy()
#         self.patch_location = patch_location
...
#         self.data = []
#         self.targets_list = []
#         self.samples = sorted(self.samples, key=lambda x: x[1])  # sort by class index
#         self.poison_samples = collections.defaultdict(lambda: False)

#         # Load images into memory (can be changed to lazy load for large datasets)
...
#         if perturb_type == 'classwise' and self.perturb_tensor.shape[0] != len(self.classes):
#             raise ValueError('Perturb tensor size mismatch for classwise poisoning.')

#         # Randomly select poisoned indices
#         total_samples = len(self.samples)
#         poison_indices = np.random.choice(total_samples, int(total_samples * poison_rate), replace=False)
...
#                 noise = self.perturb_tensor[target]
#             elif perturb_type == 'samplewise':
#                 noise = self.perturb_tensor[idx]
#             else:
#                 raise ValueError("Invalid perturb_type")

#             noise = patch_noise_extend_to_img(noise, self.data[idx].shape, patch_location=self.patch_location)
#             if add_uniform_noise:
#                 noise += np.random.uniform(0, 8, self.data[idx].shape)

#             self.data[idx] += noise
#             self.data[idx] = np.clip(self.data[idx], 0, 255)

#         self.data = [Image.fromarray(x.astype(np.uint8)) for x in self.data]

#     def __getitem__(self, index):
#         img, target = self.data[index], self.targets_list[index]
#         if self.transform is not None:
#             img = self.transform(img)
#         return img, target
...
class PoisonPubFig(datasets.ImageFolder):
    def __init__(self, root, transform=None, poison_rate=1.0, perturb_tensor_filepath=None,
        ...
        for path, target in self.samples:
            img = Image.open(path).convert("RGB")
            # img = img.resize((32, 32))  # Match expected resolution
            img = img.resize((128, 128))  # Match expected resolution
            self.data.append(np.array(img).astype(np.float32))
            self.targets_list.append(target)

        self.data = np.stack(self.data)
        self.targets_list = np.array(self.targets_list)

        # Check shape compatibility
        target_dim = self.perturb_tensor.shape[0] if len(self.perturb_tensor.shape) == 4 else self.perturb_tensor.shape[1]
        if perturb_type == 'samplewise' and target_dim != len(self.data):
            raise ValueError('Poison Perturb Tensor size not match for samplewise')
        elif perturb_type == 'classwise' and target_dim != len(self.classes):
            raise ValueError('Poison Perturb Tensor size not match for classwise')

        # Select poison targets
        self.poison_samples = collections.defaultdict(lambda: False)
        self.poison_class = []
        if poison_classwise:
            all_classes = list(range(len(self.classes)))
            if poison_classwise_idx is None:
                self.poison_class = sorted(np.random.choice(all_classes, int(len(all_classes) * poison_rate), replace=False).tolist())
            else:
                self.poison_class = poison_classwise_idx
            self.poison_samples_idx = [i for i, label in enumerate(self.targets_list) if label in self.poison_class]
        else:
            self.poison_samples_idx = sorted(np.random.choice(len(self.data), int(len(self.data) * poison_rate), replace=False).tolist())
        ...
        for idx in self.poison_samples_idx:
            ...
            if perturb_type == 'samplewise':
                ...
                noise = perturb_tensor[self.targets_list[idx]]
            else:
                raise ValueError("Unknown perturb_type")
        ...
        print('add_uniform_noise: ', add_uniform_noise)
        print(self.perturb_tensor.shape)
        print('Poison samples: %d/%d' % (len(self.poison_samples), len(self.data)))
