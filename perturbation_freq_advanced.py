#Apply simple FFT of phase and amplitude based noise in the image
import argparse
import collections
import datetime
import os
...
def universal_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, band_fft):
    ...
    while condition:
        if args.attack_type == 'min-min' and not args.load_model:
            ...
            for j in range(0, args.train_step):
                ...
                perturbed_images = fft_perturbation(images, epsilon=args.epsilon, band=band_fft)
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, band_fft):
    datasets_generator = dataset.DatasetGenerator(train_batch_size=args.train_batch_size,
                                                  eval_batch_size=args.eval_batch_size,
                                                  train_data_type=args.train_data_type,
                                                  train_data_path=args.train_data_path,
    ...
    while condition:
        if args.attack_type == 'min-min' and not args.load_model:
            ...
            for j in tqdm(range(0, args.train_step), total=args.train_step):
                ...
                perturbed_images = []
                for i, (image, label) in enumerate(zip(images, labels)):
                    # perturbed_image = fft_perturbation(image.unsqueeze(0), epsilon=args.epsilon, perturbation_type=args.perturb_type)
                    perturbed_image = fft_perturbation(image.unsqueeze(0), epsilon=args.epsilon, band=band_fft)
...
def fft_perturbation(images, epsilon=0.05, band='high'):
    """
    Apply structured FFT perturbations in frequency domain to make data unlearnable.
    
    Args:
        images: torch.Tensor of shape (N, C, H, W)
        epsilon: perturbation strength
        band: which frequency band to target: 'high', 'mid', 'low', or 'all'
    """
    B, C, H, W = images.shape
    device = images.device

    # FFT of images
    images_fft = torch.fft.fftshift(torch.fft.fftn(images, dim=(-2, -1)), dim=(-2, -1))

    # Generate noise
    noise = torch.randn_like(images_fft, dtype=torch.cfloat).to(device) * epsilon

    # Create frequency mask
    yy, xx = torch.meshgrid(torch.linspace(-1, 1, H), torch.linspace(-1, 1, W), indexing='ij')
    freq_radius = torch.sqrt(xx**2 + yy**2).to(device)  # normalized frequency magnitude
    ...
    mask = mask[None, None, :, :]  # shape to (1, 1, H, W)

    # Apply noise in frequency domain
    perturbed_fft = images_fft + noise * mask

    # Shift back and inverse FFT
    perturbed_fft = torch.fft.ifftshift(perturbed_fft, dim=(-2, -1))
    perturbed_images = torch.fft.ifftn(perturbed_fft, dim=(-2, -1)).real

    return perturbed_images.clamp(0, 1)



# def fft_perturbation(images, epsilon, perturbation_type='classwise'):
#     """
#     Apply FFT perturbations on the image tensor.
#     Perturbations are applied in the frequency domain.
#     """
#     # Apply FFT to the image batch (N, C, H, W)
#     images_fft = torch.fft.fftn(images, dim=(-2, -1))  # FFT on spatial dimensions

#     # Generate perturbations in the frequency domain
#     perturbation = torch.randn_like(images_fft).to(images.device) * epsilon

#     # Apply class-wise or sample-wise perturbation based on the perturb_type
#     if perturbation_type == 'classwise':
#         perturbation = perturbation.mean(dim=0, keepdim=True)  # Average perturbation across the batch
#     # If 'samplewise', no changes, perturbation stays unique for each image

#     # Modify the phase and amplitude
#     phase = torch.angle(images_fft)
#     amplitude = torch.abs(images_fft)
#     perturbation_amplitude = torch.abs(perturbation)
#     perturbation_phase = torch.angle(perturbation)

#     # Apply modifications
#     perturbed_amplitude = amplitude + perturbation_amplitude
#     perturbed_phase = phase + perturbation_phase

#     # Combine new amplitude and phase to create a new frequency representation
#     perturbed_fft = torch.polar(perturbed_amplitude, perturbed_phase)

#     # Apply inverse FFT to return to spatial domain
#     perturbed_images = torch.fft.ifftn(perturbed_fft, dim=(-2, -1)).real  # IFFT to get back to image domain
...
def main():
    ...
    if args.attack_type == 'random':
        ...
        if args.perturb_type == 'samplewise':
            noise = sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, args.band_fft)
        elif args.perturb_type == 'classwise':
            noise = universal_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, args.band_fft)
        torch.save(noise, os.path.join(args.exp_name, 'perturbation.pt'))
        logger.info(noise)
        logger.info(noise.shape)
        logger.info('Noise saved at %s' % (os.path.join(args.exp_name, 'perturbation.pt')))
